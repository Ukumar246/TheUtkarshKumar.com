...
function getServerTriggers() {
  ...
  data['_method'] = 'GET';
  return Parse._ajax('POST', 'https://dare-server.herokuapp.com/hooks/triggers', JSON.stringify(data)).then(
      function(res) {
...
function getServerFunctions() {
  var data = getAuthenticatedRequestData();
  data['_method'] = 'GET';
  return Parse._ajax('POST', 'https://dare-server.herokuapp.com/hooks/functions', JSON.stringify(data)).then(
      function(res) {
        res.results.forEach(function(hook) {
          if (hook.url) {
...
function registerTriggers(triggerType) {
  ...
  Webhooks.Routes[triggerType].forEach(function(item) {
    ...
    data['url'] = url;
    var path = 'https://dare-server.herokuapp.com/hooks/triggers';
...
function registerFunctions() {
  ...
  Webhooks.Routes['function'].forEach(function(item) {
    ...
    data['url'] = url;
    var path = 'https://dare-server.herokuapp.com/hooks/functions';
